...
- For classification tasks asking for multiple category outputs, we provide a calibration set containing $n$ samples, and we want the model to output a set such that  $\tau(x_{n+1}) \subseteq y$ , and

$$
P[y_{n+1} \in \tau(x_{n+1})] \ge 1 - \alpha
$$
...
$$
C(X_{\rm test}) = \left\{y : s(X_{\rm test},y) \le \hat{q}\right\}.
$$
